...
// 复制 static-site.html 到输出目录的 index.html
function copyStaticSite() {
  const staticSitePath = path.join(__dirname, 'static-site.html');
  const indexPath = path.join(__dirname, 'out', 'index.html');
  
  if (fs.existsSync(staticSitePath)) {
    // 备份原始 index.html
    if (fs.existsSync(indexPath)) {
      fs.copyFileSync(indexPath, path.join(__dirname, 'out', 'original-index.html'));
      console.log('已备份原始 index.html');
    }
    
    // 复制静态站点 HTML
    fs.copyFileSync(staticSitePath, indexPath);
    console.log('已复制静态站点到 index.html');
  } else {
    console.log('找不到静态站点文件');
  }
}
...
function main() {
  ...
  createCNAME();
  ensureNoJekyll();
  copyStaticSite(); // 最后复制静态站点，这样它会覆盖自动生成的主页
  console.log('部署准备完成');
}
